...
const Services: React.FC = () => {
  ...
  const articlesContent: Record<string, string> = {
    'automation': 'אוטומציה היא לא רק חיבור בין כלים, היא בניית ארכיטקטורה עסקית חדשה. בעזרת Make, אנחנו יוצרים זרימות נתונים שסוגרות את הפער שבין המכירות לשירות הלקוחות.',
    ...
    <section id="services" className="py-24 px-6 max-w-7xl mx-auto">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {SERVICES.map((service) => (
          <div 
            key={service.id} 
            className="glass p-10 rounded-[2.5rem] relative group border-white/5 transition-all duration-500 reveal hover:border-cyan-500/30 hover:bg-white/5 text-right"
          >
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 animate-fadeIn">
          ...
          <div className="glass max-w-2xl w-full p-12 rounded-[3rem] border-white/10 relative z-10 animate-slideUp">
            ...
            <div className="text-right" dir="rtl">
              ...
              <div className="w-12 h-1 bg-cyan-500 mb-8 ml-auto"></div>
              <p className="text-xl text-gray-300 leading-relaxed">{articlesContent[activeArticle.id]}</p>
              <a href="#contact" onClick={() => setActiveArticle(null)} className="inline-block mt-10 px-8 py-4 bg-cyan-500 text-black font-black rounded-xl hover:bg-cyan-400 transition-colors">בואו נדבר על זה</a>
            </div>
          </div>
        </div>
      )}
